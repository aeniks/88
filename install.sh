...
## installer 
install88() {
hash sudo 2>/dev/null && sudo="sudo"; hash sudo 2>/dev/null || alias sudo=' '; [ "$PREFIX" ] && unset sudo; [ $UID = 0 ] && unset sudo && unalias sudo; mkdir $HOME/{tmp,logs,gh,backup,.config} 2>/dev/null; export tmp=$HOME/tmp time=$(date +%y%m%d%H%m%S) bu=$HOME/backup; for i in ""pp{0..8}" "; do unset -v $i; done; unset -v _yno_prompt ny p pp yy 2>/dev/null; 
local IFS=$' \n\t' start=$HOME/88 green='\e[32m' dim='\e[2m' re='\e[0m' red='\e[31m' cyan='\e[36m' yellow='\e[33m' blue='\e[36m' bold='\e[1m' height="$(stty size|cut -f1 -d" ")" width="$(stty size|cut -f2 -d" ")" yno='\e[0m[\e[2mY\e[0m/\e[2mn\e[0m]' c2='    \e[0m\e[36m--\e[0m' uu="60" enter='\e[\e[0m [\e[2mENTER\e[0m] to accept or [\e[2mq\e[0m]\e[2mto Quit \e[0m' x="2>/dev/null" yy="1"; 
#####
#####
...
####
_prompt "download config-files? " && (p2 "$c2 "; p1 "where to? "; read -ei "$HOME/" "hstart"; start="${hstart}/88"; sleep .2; export start="${start/\/\///}"); 
####
_prompt "install config-files? "; 
####
####
_prompt "login to github ? "; 
########################################
########################################
####
####
...
echo; 
echo; 
if [ $pp1 ]; then $sudo apt update && $sudo apt upgrade -y; fi; 
echo; 
echo; 
...
echo; 
echo; 
if [ $pp3 ]; then _backup "$start"; _newcolor; hash -v git &>/dev/null || $sudo apt install -y git 2>/dev/null; git clone https://github.com/aeniks/88.git $start 2>/dev/null; cd "$start"; git config remote.origin.url git@github.com:aeniks/88.git; cd $OLDPWD; 
fi; 
echo; 
echo; 
...
echo; 
echo; 
if [ $pp5 ]; then ghuser="$(id -nu)"; ghmail="$(id -nu)@$(hostname)"; gh_aeniks="$start/c/gpg/gh_aeniks.gpg"; for ss in {ssh,openssl,openssh-server,gh,git} ; do hash $ss && echo $ss && break; _newcolor; $sudo apt install -y ${ss}; done; printf %b "\n\n\e[A\e[96m -- \e[0m"; gpg --pinentry-mode loopback -o "gh.txt" -d "$gh_aeniks"; gh auth login --with-token < "gh.txt"; printf "$c2 "; rm gh.txt; sleep .2; gh auth status && printf %b "\e[60G      \e[8D  "; p2 "\e[0;1m [\e[0;92m"; p1 "OK"; p2 "\e[0;1m]  \e[0m\n"; git config --global user.name $ghuser; git config --global user.email $ghmail; git config --global init.defaultBranch main; [ $(ls $HOME/.ssh/*.pub) ] || [ -r $HOME/.ssh/id_ed25519.pub ] || ssh-keygen -N '' -f $HOME/.ssh/id_ed25519; chmod 600 $HOME/.ssh/*; chmod 644 $HOME/.ssh/*.pub; gh config set git_protocol ssh; gh ssh-key add $HOME/.ssh/id_ed25519.pub; printf %b "\e[96m\u990 \e[0m"; ssh -T git@github.com; printf %b "\n"; cd $start; git config remote.origin.url git@github.com:aeniks/88.git 2>/dev/null; cd $OLDPWD; fi;
echo; 
echo; 
}; 
####
