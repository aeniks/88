...
for i in ""p{1..8}" "; do unset -v $i; done; 
#####
...
####
p1() { p2=" ${@}"; for i in $(seq ${#p2}); do sleep .02; printf %b "${p2:${i}:1}"; done; }; ## rolling text 
p2() { printf %b "$@"; }; 
####
...
[ "$1" ] && ny=${1}; 
printf %b "\e[?25h\e[40G\b\b\b\b\b\b\b\b$yno "; 
printf -v _yno_${1} "false"; read -rsn1 ny; 
[[ -z $ny || $ny = y ]] && printf -v "p$((yy++))" %b "ok" && printf -v _yno_${1} "true"; _ok;  
...
####
_prompt "download config-files? "; 
####
_prompt "install config-files? " && (
####
_backup $HOME/.inputrc; _newcolor; _link $start/c/inputrc $HOME/.inputrc; _newcolor; 
####
...
if [ $p1 ]; then 
# if [ "$_yno_prompt1" = "true" ]; then 
$sudo apt update && $sudo apt upgrade -y; 
fi; 
echo; 
echo; 
if [ $p2 ]; then for i in $(ls -1 $HOME/88/install/ap); do rr="$((RANDOM%222))";  printf %b "\ec\e[0m -- \e[48;5;${rr}m ${i} \e[0m --\n\e[38;5;${rr}m "; command -v $i &>/dev/null || $sudo apt install -y $i; done; printf %b "\e[2J\e[2H\n\n\e[0m -- \e[48;5;196m DONE \e[0m --\n\n"; fi; 
echo; 
echo; 
if [ $p3 ]; then p2 "    $c2 "; p1 "where to? "; read -ei "$HOME/" "hstart"; start="${hstart}/88"; sleep .2; export start="${start/\/\///}"; _backup "$start"; _newcolor; hash -v git &>/dev/null || $sudo apt install -y git 2>/dev/null; git clone https://github.com/aeniks/88.git $start 2>/dev/null; cd "$start"; git config remote.origin.url git@github.com:aeniks/88.git; cd $OLDPWD; fi; 
echo; 
echo; 
