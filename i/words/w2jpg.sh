...
w="$(command ls -t1 $wordlog|grep -e '.log$'|head -n1)"; ww="$wordlog/$w"; word="${w/.*/}"; local IFS=$'\n\t '; 
...
############
sed -n 3p $ww|bat -ppfljs --theme Coldark-Cold|ansifilter -M -F "monospace" -s 60 >> $wordlog/$word.htm; 
############
gum style --margin "0 2 2 2" --border hidden --padding "1 2 1 2" --border-foreground "#666666" \
"$(sed -n 4,8p $ww|fmt -w 55 -g 40|bat -pp --theme Coldark-Cold)"|ansifilter -M -F "monospace" -s 60 >> $wordlog/$word.htm; 
####
