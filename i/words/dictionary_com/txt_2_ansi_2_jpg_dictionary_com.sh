#!/bin/bash
wd="ao_dai"; 
wf="$HOME/logs/words/dictionary_com"; 
...
lina 9)" | \
ansifilter -M -s 60 -F "Cutive Mono" | tee "${wf}/${wd}.html"; 
####
(
printf %b "\e[1;38;2;244;122;166m"; 
sed -n 4p "${wf}/${wd}.log"; printf %b "\e[0m";
sed -n 2p "${wf}/${wd}.log"|bat -ppfljs --theme TwoDark; 
sed -n 5p "${wf}/${wd}.log"|fmt --goal 29|gum style --border hidden --padding "1 2 1 2" --margin "0 2 5 2" --background "#dcdcdc") | \
ansifilter -M -F "Cutive Mono" -s 50 | tee -a "${wf}/${wd}.html"; 
####
####
